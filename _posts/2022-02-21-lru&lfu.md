...
这一版本实现的缺点是查找时，先根据 key 获取该页面的访问频率，然后找到该频率的 bucket 进行顺序查找，这步操作导致查询不能达到O(1)复杂度；另外图省事使用的STL也导致了空间浪费，frequency存储了两次，key也存了两次，还存在优化空间。

搜资料的时候看到一种插入和查询都是O(1)复杂度的[实现方法](5)。比起上个版本，这个版本中第一个 map 不再是 key 到 frequency 的映射了，而是 key 到 bucket 中页面的具体存储位置的映射，也就是可以直接通过 key 找到页面。访问页面之后可以通过 parent 指针直接将页面放入下一个 bucket 中，避免了之前顺序查找页面的开销。
...
另外一点是这里计算使用频率并不是真的每访问一次，counter 就会加一。Redis 这部分实现的逻辑是，counter 最大为255,当counter 越接近255,每次访问时 counter 实际加一的概率越小。具体实现参见 `ecivt.c` 中的代码片段。
```
