...
数据库系统开始恢复数据时，首先查看日志记录，重做所有已经提交的事务，撤销所有还未完成的事务。由于我们是先写日志然后再写数据库，所以如果一项修改被应用到数据库那么它肯定会出现在日志中，为了保证数据库状态的一致必须重根据日志重做对应的操作；同时，对于未提交的事务（没有$<T_i, abort> / <T_i, commit>$标识的事务），我们需要undo所进行的修改。  
// TODO：重做 abort 事务似乎是额外开销，实际是否真的需要重做？
...
> 出于性能考虑，目前大部分数据库采用的是 Steal, No Force 的策略。
...
内存中分配有 2MB 的 double write 缓冲，磁盘上也分配有连续的 2MB double write 共享表空间。每次刷盘时先把脏页存到内存中的缓冲区，缓冲满时先顺序写入 double write 磁盘中的共享表空间，写入共享表的工作完成后再将数据页写到实际的存储位置。这样当数据页发生 partial write 时，总能从 double write 的共享表空间找到完整的页面数据。
