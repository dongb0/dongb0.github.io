...
- 情况一：修改 k/v 产生的 dirty page
  ...
  ![update-kv-dirty](/img/in-post/post-boltdb/b-plus-tree-update-dirty.png)

- 情况二：插入或删除 k/v 产生的 dirty page

  另外写事务中对 k/v 进行增删都可能引起索引结构的改变，索引中相关的页面都会变成 dirty page，需要先拷贝原有 page，修改后写到新页面中，这一过程需要不断向上递归进行，哪怕某个节点中容量足够，但是因为 inode 中指针改变了（因为子页面改变了），该节点依然变为 dirty，就变为上一段描述的情况。

  // TODO： 分裂 或 rebalance 的产生 dirty page 的示意图
...
2. 判断 node 对应页面是否为旧页面，如果是旧的则将该页面放入 freeList 中表示可以被回收（关于 freeList 我们将在[这里]()展开介绍，因为接下来我们马上会将这个页面的内容写入一个新页面中；
...
// TODO： 图  cursor 的 stack？

### 后悔药：Rollback 

// TODO： 待补充

那么关于 BoltDB 事务的介绍就到这里了。这是我第一次见到 shadow paging 的实现，同时还见到 B+ 树不同的实现方法。对了，关于 BoltDB 中 B+ 树的实现这里没有提到太多，除了前面提到插入导致的 split，还有删除导致的 rebalance 操作，就留到下一篇文章中具体介绍吧。今天就到这里了。
