...
> Q：现实中我们是否在使用 Strict 2PL（S2PL） 或 Strong Strict 2PL（SS2PL）？ 
> A：尽管教材提到 S2PL 在商用数据库系统中使用广泛，但目前许多商业数据库都使用 MVCC 而非基于锁的并发控制，因此现在现实中还在使用 2PL 的数据库应当相对而言不如 MVCC 的多。  
...
**Serializable**：strict 2PL，加上索引锁；事务开始时要给所有数据项加锁。（索引锁我们在 [Phantom Problom][1] 中简要介绍）
...
[^1]: [数据库系统概念第六版-14章]()
[^2]: [数据库系统概念第六版-15章]()
[1]: https://dongb0.github.io/2022/03/27/phantom/#索引锁index-locking
